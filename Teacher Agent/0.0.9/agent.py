...
messages = [HumanMessage(content="generate 3 simple addition questions")]
messages = graph.invoke({"messages": messages}, config)
for m in messages['messages']:
    m.pretty_print()
messages = [HumanMessage(content="generate 3 simple science questions")]
messages = graph.invoke({"messages": messages}, config)
for m in messages['messages']:
    m.pretty_print()
messages = [HumanMessage(content="current score of student agent 1")]
messages = graph.invoke({"messages": messages}, config)
for m in messages['messages']:
    m.pretty_print()