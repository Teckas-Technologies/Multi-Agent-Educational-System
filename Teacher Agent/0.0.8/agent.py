...
def generate_questions(state: State,env: Environment) -> str:
    ...
    "content": f"You are a teacher agent in a virtual classroom. Your sole responsibility is to guide users through interactive learning by "
               ...
               f"**If a user asks for an update on student scores or overall performance, provide the response in a table format:**\n\n"
               f"**🏆 Leaderboard**\n"
               ...
               f"**If a user asks about overall student performance, how a student can improve, or advice on competing better, respond with:**\n"
               f"- Provide a summary of the current student rankings using a leaderboard format:\n\n"
               f"**🏆 Leaderboard**\n"
               f"| Student Name | Total Score | Rank |\n"
               f"|--------------|------------|------|\n"
               f"| Student 1    | {state.student_1_total_score} | X |\n"
               f"| Student 2    | {state.student_2_total_score} | Y |\n\n"
               f"- If asked about improvement, suggest strategies such as revisiting weaker topics, practicing more questions, and maintaining consistency.\n"
               f"- If asked about competing better, encourage students to review past mistakes, participate actively, and aim for accuracy and speed in answering questions.\n"
               f"- Keep responses concise, focusing on motivation and actionable advice without unnecessary explanations.\n\n"
...
def call_student_agents(state: State, env: Environment, questions: str) -> None:
    ...
    student_agents = [
        ...
        ("luna_grilka.near", "student-agent-2", "0.0.1"), 
    ]
...
def evaluate_answers(env: Environment, state: State) -> str:
    ...
    print("Student 1 Answers:")
    print(state.student_1_answers, "\n")

    print("📌 Student 2 Answers:")
    print(state.student_2_answers, "\n")

    eval_prompt = f"""
     You are an AI evaluator. Assess student responses strictly based on correctness and context.
    ...
    **Provide the response in the exact table format below:**
    
    ## 🏆 **Leaderboard**
    
    | AgentName   | TotalQuestions | TotalScore |
    |-------------|----------------|------------|
    | Student 1   | {total_questions}         | X          |
    | Student 2   | {total_questions}         | Y          |

    **Feedback:**
    - Review all answers to ensure they directly address the questions.
    - For incorrect answers, suggest revisiting the topic and practicing similar questions.
    - For partial answers, encourage improving clarity and depth of response.
